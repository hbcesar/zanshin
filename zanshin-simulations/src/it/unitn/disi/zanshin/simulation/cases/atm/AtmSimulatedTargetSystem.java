...
public class AtmSimulatedTargetSystem extends SimulatedTargetSystem {
	/** The object in which the simulation thread is sleeping, and therefore used to "wake it up". */
	private Object lock;

	/** Constructor. */
	public AtmSimulatedTargetSystem(Object lock) {
		this.lock = lock;
	}
	...
	public void initiate(Long sessionId, String reqName) {
		...
		// strategy is selected to deal with the failures of AR1 and AR3.
		synchronized (lock) {
			lock.notifyAll();
		}
	}
